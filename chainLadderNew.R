chainladderNew <- function(Triangle, weights=1,
  ...
  ## ST added#######
  # function to filter for a specified number of prior year (orign.incl)
  f<-function(i, Tri, origin.incl,y=FALSE){
    a<-length(Tri[,i])
    b<-min(a,origin.incl)
    #print(a-b+1-i)
    Tri[max(1,a-b-i+1+y):a,i]
  }
  #########
    
  lmCL <- function(i, Triangle){
    lm(y~x+0, weights=f(i,weights,origin.incl)/f(i,Triangle,origin.incl)^delta[i],
       data=data.frame(x=f(i,Triangle,origin.incl), y=f(i+1,Triangle,origin.incl,TRUE)))
  }
